...
const init = function () {
    ...
    // WebInspector.Main._instanceForTest._socket = socket;

    slaveSocketInstance = slaveSocket(SDK);

    // 启动的时候主动建立第一个 slave 连接 , 默认 slaveId = 10   fixme: 有问题,不再设置
    // 假设slave页面提前打开了，但是后台的 socket 服务还未启动，因此需要预设第一次打开页面的 slaveId = 10
    slaveSocketInstance.connectNewSlaveSocket().then(connection => {
    ...
    const delElementPanel = function (index) {
        ...
        let elements = document.querySelectorAll('#elements-content>div');
...
window.isSmartApp = !!Runtime._queryParamsObject['fport'];

if (isSmartApp) {
    repeatFn();
}

// isSmartApp = false; // 不过滤 element
